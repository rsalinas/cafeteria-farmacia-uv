...
def extraer_menu_qrcarta(url):
    ...
    try:
        ...
        info_restaurante = {"nombre": None, "direccion": None, "telefono": None}
            ...
            while current and current.name != "h4":
                if current.name == "p" and current.get("class") and "plato" in current.get("class"):
                    ...
                    if nombre_plato:
                        ...
                        platos.append({"nombre": nombre, "alergenos": alergenos})

                current = current.find_next_sibling()

            if platos:
                menu_data["categorias"].append({"nombre": nombre_categoria, "platos": platos})
